...
var LEE = {
	...
	showSearchForm: function(DOM) {
		var shareContent = '<div id="jumbo-search-container" class="layoutMultiColumn-container"><header class="layoutMultiColumn-header"><form action="' + PURE.home_url + '" method="get" role="search"><input class="js-search-input textInput textInput--borderless textInput--jumbo" name="s" placeholder="输入搜索内容" autocomplete= "off"></form></header></div>';
	...
	openUserActions: function(DOM) {
		var userActions = '<div class="popover-inner popover-inner--followButton fontSmooth"><ul class="list list--borderless list--short list--large"><li class="list-item list-item--noPadding"><a class="button--link" href="/me/bookmarks">收藏</a></li><li class="list-item list-item--noPadding"><a class="button--link" href="/me/settings">资料</a></li><li class="list-item list-item--noPadding"><a class="button--link" href="' + PURE.login_out_url + '">注销</a></li></ul></div>';
		LEE.makePopover(DOM, userActions);
	},
	openLoginForm: function(DOM) {
		var loginForm = '<div class="popover-inner popover-inner--followButton fontSmooth">';
		if (PURE.is_oauth_sina) loginForm += '<button class="login-social__weibo login-social js-action" data-action="oauthSina"><i class="icon-weibo iconfont"></i>使用微博登录</button>';
		if (PURE.is_oauth_qq) loginForm += '<button class="login-social__qq login-social js-action" data-action="oauthQQ"><i class="icon-qq iconfont"></i>使用QQ登录</button>';
		if (PURE.is_oauth_github) loginForm += '<button class="login-social__github login-social js-action" data-action="oauthGithub"><i class="icon-github iconfont"></i>使用Github登录</button>';
		loginForm += '</div>';
	...
	oauthSina: function() {
		window.open(PURE.oauth_sina, "微博授权登录", box);
		jQuery(".popover").remove();
	},
	oauthGithub: function() {
		window.open(PURE.oauth_github, "Github授权登录", box);
		jQuery(".popover").remove();
	},
	...
	imageZoom: function(splendidImage, splendidImageUrl, splendidPagesize) {
		var splendidNewImage = new Image();
		splendidNewImage.onload = function() {
			var pagesize = splendidPagesize,
				x = pagesize[0] - 100,
				y = pagesize[1] - 100,
				img_w = splendidNewImage.width,
				img_h = splendidNewImage.height;
			if (img_w > x) {
				img_h = img_h * (x / img_w);
				img_w = x;
				if (img_h > y) {
			...
			jQuery(splendidImage).attr("src", splendidImageUrl).css({
				width: img_w,
				height: img_h,
				"margin-left": a,
				"margin-top": margintop
			}).addClass("appearfromup");
		};
		splendidNewImage.src = splendidImageUrl;
	},
	imageZoomIn: function(DOM) {
		var splendidImageUrl = DOM.attr("href"),
			pagesize = LEE.getPageSize();
		if (typeof document.body.style.maxHeight === "undefined") {
			alert("\u4E0D\u652F\u6301IE6\uFF01\u8BF7\u4F60\u4EEC\u653E\u8FC7IE6\u5427\uFF0C\u5B83\u592A\u8001\u4E86\uFF0C\u5C31\u8BA9\u5B83\u5B89\u5FC3\u7684\u53BB\u5427\u2026\u2026");
			return false
		} else {
			pureBody.append('<div id="splendid-wrap" class="splendid-wrap js-action" data-action="imageZoomOut"><img id="splendid-img" class="splendid-image js-action" data-action="imageZoomOut" /></div>');
			pureBody.addClass('is-loadingApp');
			LEE.imageZoom("#splendid-img", splendidImageUrl, pagesize);
			jQuery("#splendid-img").load(function() {
				pureBody.removeClass('is-loadingApp');
			});
		}
		return false
	},
	imageZoomOut: function(DOM) {

		pureBody.removeClass('is-loadingApp');
		jQuery("#splendid-wrap,#splendid-img").unbind("click");
		jQuery("#splendid-wrap").fadeOut(400, function() {
			jQuery(this).remove()
		});

	},
	showSharePopover: function(DOM){
		var content = '<div class="popover-inner popover-inner--menu"><ul class="list list--borderless list--short sharePopover--simple"><li class="list-item"><button class="button button--chromeless button-share js-action" data-action-value="twitter" data-action="share"><span class="iconfont icon-twitter"></span>分享到Twitter</button></li><li class="list-item"><button class="button button--chromeless button-share js-action" data-action-value="weibo" data-action="share"><span class="iconfont icon-weibo"></span>分享到微博</button></li><li class="list-item"><button class="button button--chromeless button-share js-action" data-action-value="qq" data-action="share"><span class="iconfont icon-qq"></span>分享到QQ空间</button></li></ul></div>';
		LEE.makePopover(DOM, content);
	},
	share: function(DOM){
		var data = $(".button--share"),
	d = DOM.data("action-value"),
	e = data.data('title'),
	f = data.data('url'),
	g = data.data('image'),
	h = "";
	...
	window.open(h, "Share", box);	
	$('.popover').remove();
	
	},
	addToBookmarks: function(DOM) {
	if (DOM.hasClass('is-activing')) {
			return false;
		} else {	
		DOM.addClass('is-activing');		
		var id = DOM.data("id");
		var ajax_data = {
		...
		pureBody.addClass('is-loadingApp');
			$.ajax({
				...
				success: function(data) {
					pureBody.removeClass('is-loadingApp');
					DOM.removeClass('is-activing');
					if (data.status == 200) {
						createButterbar(data.data);
						DOM.addClass('is-bookmarked');
						DOM.attr('data-action','removeFromBookmarks');
						DOM.data('action','removeFromBookmarks');
						var bookmarkStatus = DOM.find('.bookmarkStatus-message');
						if( bookmarkStatus.length > 0 ){
							bookmarkStatus.html('Bookmarked');
						}
					} else {
						createButterbar(data.data);
					}
	...
	removeFromBookmarks: function(DOM) {
	if (DOM.hasClass('is-activing')) {
			return false;
		} else {	
		DOM.addClass('is-activing');		
		...
		pureBody.addClass('is-loadingApp');
			$.ajax({
				...
				success: function(data) {
					...
					if (data.status == 200) {
						createButterbar(data.data);
						DOM.removeClass('is-bookmarked');						
						DOM.attr('data-action','addToBookmarks');
						DOM.data('action','addToBookmarks');
						var bookmarkStatus = DOM.find('.bookmarkStatus-message');
						if( bookmarkStatus.length > 0 ){
							bookmarkStatus.html('Bookmark');
						}
					} else {
						createButterbar(data.data);
					}
